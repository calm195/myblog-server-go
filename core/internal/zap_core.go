...
func NewZapCore(level zapcore.Level) *ZapCore {
	...
	entity.Core = zapcore.NewCore(global.BlogConfig.Zap.Encoder(), syncer, levelEnabler)
	return entity
}
...
func (z *ZapCore) WriteSyncer(formats ...string) zapcore.WriteSyncer {
	cutter := NewCutter(
		...
		z.level.String(),
		global.BlogConfig.Zap.RetentionDay,
		CutterWithLayout(time.DateOnly),
		CutterWithFormats(formats...),
	)
	if global.BlogConfig.Zap.LogInConsole {
...
func (z *ZapCore) Write(entry zapcore.Entry, fields []zapcore.Field) error {
	for i := 0; i < len(fields); i++ {
		if fields[i].Key == "business" || fields[i].Key == "folder" || fields[i].Key == "directory" {
			...
			z.Core = zapcore.NewCore(global.BlogConfig.Zap.Encoder(), syncer, z.level)
		}
	}
